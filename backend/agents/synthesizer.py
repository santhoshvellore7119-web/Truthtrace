...
Synthesizes findings from OSINT Hunter, Wayback CDX, Fact Checker, Narrative Profiler,
Red-Team Auditor, and Video Analyst into a unified, chronologically sorted forensic Dossier.
"""
...
class SynthesizerAgent(BaseAgent):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> AgentResult:
        ...
        try:
            ...
            claims_corpus = self._build_claims_corpus(claims, provenance)
            vector_store.add_claims(claims_corpus)

            # 2. Build sorted chronological timeline
            timeline = self._build_timeline(provenance, wayback_snapshots)

            # 3. Determine overall verdict & confidence
            verdict, confidence = self._determine_overall_verdict(fact_check_results, red_team_audit)

            # 4. Extract Patient Zero candidate from earliest timeline event
            patient_zero = self._extract_patient_zero(timeline, provenance)

            # 5. Build SubClaims and Evidence
            sub_claims = self._build_sub_claims(claims, fact_check_results, provenance, wayback_snapshots, verdict, confidence)
    ...
    def _extract_patient_zero(self, timeline: List[TimelineEvent], provenance: List[Dict]) -> Optional[OriginatingAccount]:
        """Identify candidate Patient Zero from the earliest timeline event."""
        if not timeline:
            return None
        ...
        # Check if handle is present in metadata or URL
        if earliest.metadata and 'earliest_mention' in earliest.metadata:
        ...
        return OriginatingAccount(
            platform=platform,
            handle=handle,
            first_seen_at=earliest.earliest_cdx_timestamp or earliest.timestamp or datetime.now(),
    ...
    def _parse_datetime(self, val: Any) -> Optional[datetime]:
        """Safely parse various datetime strings."""
